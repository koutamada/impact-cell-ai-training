...
Supabaseの標準SMTPには送信先と送信数の制限があり、任意の宛先を用いた本番相当の配信試験には適さない場合がある。

追加確認として、完全な新規アカウントへ確認メールが届くこと、確認メールのリンクが機能すること、確認完了後にログインできることを公開環境の実ブラウザで確認した。また、パスワード再設定メールの受信、再設定リンクからの復帰、新しいパスワードの設定までのフローが機能することを確認した。これにより、公開環境の新規登録メールとパスワード再設定メールに関する未確認事項を解消した。
