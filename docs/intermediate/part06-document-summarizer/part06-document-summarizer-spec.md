...
アカウント作成後にsessionが発行されない場合、新規利用者のメール確認待ちと既存アカウントの再登録をクライアントから区別しない。登録結果画面には見出し「登録手続きを受け付けました」と説明「確認メールが届いた場合は、メール内のリンクを開いてください。すでに登録済みの場合はログインしてください。」を共通表示する。Supabase Authの応答からアカウントの存在を推測したり、登録済みであることを個別表示したりしない。
